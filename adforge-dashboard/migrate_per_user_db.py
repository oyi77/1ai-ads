...
def migrate_user(user_id, username):
    ...
    try:
        ...
        for acc in accounts:
            existing = uc.execute(
                "SELECT id FROM platform_accounts WHERE id = ?", (acc["id"],)
            ).fetchone()
            if not existing:
                try:
                    uc.execute(
    ...
    try:
        ...
        if account_ids:
            ...
            camps = master.execute(
                f"SELECT * FROM campaigns WHERE campaign_id IN ({placeholders})",
                account_ids,
            ).fetchall()
            for camp in camps:
                existing = uc.execute(
                    "SELECT id FROM campaigns WHERE id = ?", (camp["id"],)
                ).fetchone()
                if not existing:
                    try:
                        uc.execute(
    ...
    try:
        drafts = mc.execute(
            "SELECT * FROM approval_drafts WHERE user_id = ?", (user_id,)
        ).fetchall()
        for draft in drafts:
            existing = uc.execute(
                "SELECT id FROM approval_drafts WHERE id = ?", (draft["id"],)
            ).fetchone()
            if not existing:
                try:
                    uc.execute(
...
def main():
    ...
    print(f"\n{'='*50}")
    print(
        f"Migration complete: {total_migrated} items migrated across {len(users)} users."
    )
