...
export function createWhatsappIntelligenceGroupRouter({ services }) {
  ...
  // Authenticated API endpoints, scoped per tenant: every handler below
  // receives req.user.id and the repository filters by it, so a tenant only
  // ever reads or acts on their own conversations.
