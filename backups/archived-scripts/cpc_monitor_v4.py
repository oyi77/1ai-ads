...
GAS_CPC = 100; OK_CPC = 130; WARN_CPC = 150
CTR_GAS = 5.0; CTR_OK = 3.0; CTR_WARN = 3.5
...
def get_insights(cid, preset='today'):
    data = api_get(f'{cid}/insights', {'fields': 'spend,cpc,ctr,impressions,clicks', 'date_preset': preset, 'limit': 1})
    rows = data.get('data', [])
...
def scan_dead_campaigns(act_id, act_label, state):
    ...
    for d in rows:
        ...
        if impr > 0 and cpc > 0 and cpc <= CPC_LIMIT and ctr >= CTR_OK:
            ...
            result = api_post(cid, {'name': new_name, 'status': 'ACTIVE', 'daily_budget': DEFAULT_BUDGET})
            if result.get('success'):
                reactivated_count += 1
                log(f"🔄 [{act_label}] DEAD→CBO_ON: {cname[:40]:40s} | CPC {cpc:.0f} | CTR {ctr:.2f}%", LOG_FILE)
...
def check_and_pause(cid, cname, act_label, state):
    ...
    if cpc > CPC_LIMIT:
        ...
        state['paused'][cid] = {
            ...
            'cpc': cpc, 'ctr': ctr, 'spend': spend,
            'death_count': dc, 'account': act_label, 'name': cname
        }
        
        result = api_post(cid, {'name': f'PAUSED_CPC{cpc:.0f}_{clean_name(cname)}'[:50], 'status': 'PAUSED'})
        if result.get('success'):
            log(f"🛑 [{act_label}] PAUSED: {cname[:40]} | CPC {cpc:.0f} | Strike {dc}", LOG_FILE)
            if dc >= DEATH_LIMIT:
...
def check_and_reactivate(state):
    ...
    for cid, info in list(state.get('paused', {}).items()):
        ...
        if cpc <= GAS_CPC and ctr >= CTR_GAS:
            ...
            api_post(cid, {'name': new_name, 'status': 'ACTIVE', 'daily_budget': DEFAULT_BUDGET})
            del state['paused'][cid]
            log(f"🟢 [{label}] GAS: {name[:30]} → CPC {cpc:.0f} | CTR {ctr:.2f}%", LOG_FILE)
...
def ensure_budgets(act_id, act_label):
    ...
    for c in active:
        ...
        if current != DEFAULT_BUDGET:
            ...
            if result.get('success'):
                log(f"💰 [{act_label}] {c['name'][:35]:35s} Rp{current:,} → Rp{DEFAULT_BUDGET:,}", LOG_FILE)
