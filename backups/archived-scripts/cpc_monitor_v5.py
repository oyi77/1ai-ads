...
def process_account(act_id, label, state):
    ...
    for c in active:
        ...
        if cpc > CPC_LIMIT:
            ...
            log(
                f"  ✅ {cname[:45]:45s} | CPC {cpc:>5.0f} | CTR {ctr:>5.2f}% | Rp{spend:,}",
                LOG_FILE,
            )
            # Ensure budget
            cur = int(c.get("daily_budget", 0))
    ...
    for c in paused:
        ...
        if impr > 0 and cpc > 0 and cpc <= CPC_LIMIT and ctr >= CTR_OK:
            ...
            r = api_post(
                cid, {"name": nname, "status": "ACTIVE", "daily_budget": DEFAULT_BUDGET}
            )
            if r.get("success"):
                log(
                    f"🔄 DEAD→CBO_ON: {cname[:35]:35s} | CPC {cpc:.0f} | CTR {ctr:.2f}%",
                    LOG_FILE,
                )
    ...
    for cid, info in list(state.get("paused", {}).items()):
        if info.get("account") != label:
            continue
        if info.get("death_count", 1) > DEATH_LIMIT:
            continue

        paused_at = datetime.fromisoformat(info["paused_at"])
        if (now - paused_at).total_seconds() / 60 < COOLING_MIN:
            continue
