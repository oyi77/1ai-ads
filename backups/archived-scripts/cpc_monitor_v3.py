...
def check_and_pause(cid, cname, act_label, state):
    ...
    if cpc > CPC_LIMIT:
        ...
        if result.get("success"):
            log(
                f"🛑 [{act_label}] PAUSED: {cname} | CPC {cpc:.0f} | Strike {dc} | Saved Rp{spend:,}",
                LOG_FILE,
            )
            if dc >= DEATH_LIMIT:
...
def check_and_reactivate(state):
    ...
    for cid, info in list(state.get("paused", {}).items()):
        ...
        if cpc <= GAS_CPC and ctr >= CTR_GAS:
            ...
            del state["paused"][cid]
            log(
                f"🟡 [{label}] REACTIVATE: {name} | CPC {cpc:.0f} | CTR {ctr:.2f}%",
                LOG_FILE,
            )
            ...
            del state["paused"][cid]
            log(
                f"💀 [{label}] DEAD: {name} | CPC {cpc:.0f} > 130 after {COOLING_MIN}m",
                LOG_FILE,
            )
...
def redistribute_budget(state):
    for act_id, act_cfg in ACCOUNTS.items():
        ...
        for c in active:
            ...
            if result.get("success"):
                log(
                    f"💰 [{label}] {c['name'][:30]}: Rp{old:,} → Rp{old+per_c:,}",
                    LOG_FILE,
                )
