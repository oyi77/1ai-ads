...
TOKEN = os.environ.get("FB_ACCESS_TOKEN_1041", os.environ.get("META_ACCESS_TOKEN", ""))
ACCOUNT = os.environ.get("FB_ACCOUNT_1041", "act_380721031313330")

# === THRESHOLDS (data-driven) ===
DAILY_HARD_CAP = int(
    os.environ.get("DAILY_HARD_CAP", "200000")
)  # Hard stop at Rp 200k (NEW - data-driven reduction)
WARNING_AT = int(os.environ.get("WARNING_AT", "150000"))  # Warning at Rp 150k
AUTO_RESUME_AT = int(
    os.environ.get("AUTO_RESUME_AT", "100000")
)  # Auto-resume when spend drops below this
...
def pause_all(message=""):
    ...
    for c in campaigns:
        ...
        try:
            api_post(
                f"https://graph.facebook.com/v19.0/{cid}?access_token={TOKEN}",
                {"status": "PAUSED"},
            )
...
def main():
    if not TOKEN:
        print("ERROR: Set FB_ACCESS_TOKEN_1041 or META_ACCESS_TOKEN env var")
        sys.exit(1)
    state = load_state()
    ...
    try:
        ...
        s_data = api(s_url)
        spend = (
            float(s_data.get("data", [{}])[0].get("spend", 0))
            if s_data.get("data")
            else 0
        )
    ...
    log(
        f"📉 Settlement rate: {confirmed_ratio*100:.1f}% (Rp 16,124 confirmed / Rp 901,357 pending)"
    )
    ...
    if confirmed_ratio < CRITICAL_RATIO and spend > 100000:
        log(
            f"🆘 EMERGENCY: Settlement rate {confirmed_ratio*100:.1f}% < {CRITICAL_RATIO*100:.0f}% | Commission not converting"
        )
    ...
    # Condition C: WARNING ZONE — spend approaching limit
        ...
        if is_peak and state.get("paused"):
            ...
            log(
                f"⏰ OFF-PEAK ({current_hour}:00) + spend near limit → Pausing to save for peak"
            )
            pause_all(
                f"Off-peak save — Rp {spend:,.0f} spent, saving Rp {DAILY_HARD_CAP-spend:,} for peak"
            )
            state["paused"] = True
            state["last_pause"] = datetime.now().isoformat()
            ...
            log(
                f"⏸️ State: {'PAUSED' if state.get('paused') else 'RUNNING'} | Nothing to change"
            )
        ...
        if state.get("paused"):
            ...
            if should_resume:
                if confirmed_ratio < CRITICAL_RATIO:
                    log(
                        f"⚠️ Skipping resume: settlement rate {confirmed_ratio*100:.1f}% still critical"
                    )
                ...
                log(
                    f"⏸️ Stay paused (spent Rp {spend:,.0f} > auto-resume Rp {AUTO_RESUME_AT:,})"
                )
    ...
    if confirmed_ratio < 0.10:
        log(
            f"💡 ALERT: Settlement rate {confirmed_ratio*100:.1f}% — 90%+ of commission is pending. Check settlement before scaling."
        )
    else:
    ...
    log(
        f"  Peak:     {'YES' if is_peak else 'NO'} ({PEAK_HOURS[0]}:00-{PEAK_HOURS[1]}:00)"
    )
...
if __name__ == "__main__":
    ...
    if "--daemon" in sys.argv:
        ...
        log(
            f"🚀 ADS 1041 GOVERNOR DAEMON STARTED (systemd mode) — check every {CHECK_INTERVAL}s"
        )
        while RUNNING:
            try:
                main()
