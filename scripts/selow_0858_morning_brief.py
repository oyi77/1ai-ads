...
def run_morning_brief():
    ...
    data_yesterday = {
        ...
        "avg_cpc": 130,
        "campaigns": [
            {"name": "CBO_WinningScale_BIDCAP180", "spend": 65000, "clicks": 540, "ctr": 7.8, "cpc": 120},
            {"name": "CBO_Dongkrakelektrik_All", "spend": 45000, "clicks": 380, "ctr": 5.2, "cpc": 118},
            {"name": "CBO_1-1-1_rakpiringpengering", "spend": 25000, "clicks": 180, "ctr": 0.45, "cpc": 139},
            {"name": "ADFORGE_Testing_Rak", "spend": 10250, "clicks": 20, "ctr": 0.8, "cpc": 512}
        ]
    }
    ...
    print(f"🖱 Total klik: {data_yesterday['total_clicks']:,} | CPC avg: Rp{data_yesterday['avg_cpc']}")
    print("\nCAMPAIGN STATUS:")
    ...
    for c in data_yesterday['campaigns']:
        ...
        if "rakpiringpengering" in c['name'].lower() and c['cpc'] > 100:
            recommendations.append(f"PAUSE/Turun Budget: {c['name']} - Intel CVR 0.3% (Zonasi Merah).")
        if status == "✅" and ("tiplessalad" in c['name'].lower() or "kancingjepit" in c['name'].lower() or "WinningScale" in c['name']):
            recommendations.append(f"SCALE UP: {c['name']} - Performa joss + Intel CVR bagus.")
        if status == "🚨":
