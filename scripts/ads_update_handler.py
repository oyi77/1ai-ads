...
def add_revenue(source, amount, note=""):
    ...
    with open(report_path, "a") as f:
        f.write(
            f"[{datetime.now().strftime('%Y-%m-%d %H:%M')}] {source}: Rp{amount:,} - {note}\n"
        )
...
def add_note(campaign, note):
    ...
    notes[campaign].append(
        {"time": datetime.now().strftime("%Y-%m-%d %H:%M"), "note": note}
    )
...
def add_status_update(account, message):
    ...
    if account not in statuses:
        statuses[account] = []
    statuses[account].append(
        {"time": datetime.now().strftime("%Y-%m-%d %H:%M"), "message": message}
    )
...
def generate_combined_report():
    ...
    report = {
        ...
        "recent_revenue": (
            data["revenue_entries"][-5:] if data["revenue_entries"] else []
        ),
...
def main():
    if len(sys.argv) < 2:
        print("Usage:")
        print(
            "  python3 scripts/ads_update_handler.py revenue --source NAME --amount AMOUNT [--note NOTE]"
        )
        print(
            "  python3 scripts/ads_update_handler.py note --campaign NAME --note NOTE"
        )
        print(
            "  python3 scripts/ads_update_handler.py status --account NAME --message MSG"
        )
        print(
            "  python3 scripts/ads_update_handler.py report  # generate combined report"
        )
        return
