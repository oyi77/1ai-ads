...
def get_ads_hist():
    ...
    camps = (
        requests.get(
            url, params={"access_token": ACCESS_TOKEN, "fields": "id,name,status"}
        )
        .json()
        .get("data", [])
    )
    ...
    for c in camps:
        ...
        if ins_r:
            ...
            tag = "UNKNOWN"
            if "RAK" in c["name"].upper():
                tag = "rakdapur3"
            elif "MULTI" in c["name"].upper():
                tag = "multistorage"
            elif "SOFA" in c["name"].upper():
                tag = "sofaarabian"
            ...
            if (
                roi < 0.5 and spend > 50000
            ):  # ROI < 0.5 is safe threshold for 'very bad'
                ...
                new_name = (
                    f"ADFORGE_{c['name']}"
                    if not c["name"].startswith("ADFORGE")
                    else c["name"]
                )
                requests.post(
                    f'https://graph.facebook.com/v19.0/{c["id"]}',
                ...
                actions.append(
                    f"WINNER DETECTED (ROI {roi:.2f}): {c['name']} -> Labelled ADFORGE"
                )
