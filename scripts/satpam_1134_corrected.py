...
def load_token():
  for line in open(ENV_PATH).read().splitlines():
    if not line or line.startswith('#'):
      continue
    if line.split('=', 1)[0] == 'META_ACCESS_TOKEN' and line.split('=', 1)[1].strip():
...
for c in non_off:
  i = ins_map.get(c['id'], {})
  spend = spend_value(i.get('spend'))
  cpc = cpc_value(i.get('cpc'))
  clicks = int(i.get('clicks') or 0)
  impr = int(i.get('impressions') or 0)
  ctr = ctr_value(i.get('ctr'))
  name = c.get('name', '')
  spend_total += spend
...
for item in kill:
  action = item.get('cpc_action')
  cid = item['id']
  if action == 'PAUSE':
    payload = urllib.parse.urlencode({'status': 'PAUSED', 'access_token': TOKEN}).encode()
    req = urllib.request.Request(f'{API}/{cid}', data=payload, method='POST')
    try:
      with urllib.request.urlopen(req, timeout=30) as r:
    ...
    time.sleep(1.5)
  elif action == 'OFF_/PAUSE':
    payload = urllib.parse.urlencode({'name': 'OFF_' + item['name'], 'access_token': TOKEN}).encode()
    req = urllib.request.Request(f'{API}/{cid}', data=payload, method='POST')
    try:
      with urllib.request.urlopen(req, timeout=30) as r:
    ...
    try:
      ...
      r2 = False
    time.sleep(1.5)
    item['pause_result'] = {'rename': bool(r1), 'pause': bool(r2)}

report = {
  'timestamp': datetime.now().isoformat(),
  'note': 'Using act_1773760133153789 as 1134 Glowscent alt from token-scoped adaccounts.',
  'used_act': ACT,
