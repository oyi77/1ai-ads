...
class CampaignAutoscaler:
    ...
    def load_data(self):
        ...
        click_files = sorted(DATA_DIR.glob("WebsiteClickReport*.csv"), 
                           key=lambda x: x.stat().st_mtime, reverse=True)
        order_files = sorted(DATA_DIR.glob("AffiliateCommissionReport*.csv"), 
                           key=lambda x: x.stat().st_mtime, reverse=True)
    ...
    def calculate_metrics(self):
        ...
        indonesia_clicks = len(self.clicks_df[self.clicks_df['Wilayah Klik'] == 'Indonesia'])
        ...
        product_metrics = self.orders_df.groupby('Nama Barange').agg({
            'ID Pemesanan': 'count',
        ...
        product_metrics['Cancel Rate'] = (product_metrics['Cancel Count'] / 
                                        product_metrics['Order Count'] * 100)
        
        # Platform performance
        platform_performance = self.orders_df.groupby('Platform').agg({
            'ID Pemesanan': 'count',
            'Total Komisi per Pesanan(Rp)': 'sum'
        }).rename(columns={
            'ID Pemesanan': 'Order Count',
    ...
    def identify_opportunities(self):
        ...
        if len(high_cancel) > 0:
            ...
            for _, row in high_cancel.head(5).iterrows():
                print(f"    - {row['Nama Barange'][:40]}: {row['Cancel Rate']:.1f}% cancel rate")
        ...
        print(f"  Best performing platform: {max_platform[0]} ({max_platform[1]:,} orders)")
    ...
    def generate_optimization_plan(self):
        ...
        total_commission = sum(p['Total Commission'] for p in self.analysis['top_products'][:10])
...
Platform Performance:
"""
        for platform, count in sorted(self.analysis['orders_by_platform'].items(), 
