...
GOVERNOR_SCRIPT = os.path.join(
    os.path.expanduser("~"),
    ".openclaw",
    "workspace",
    "scripts",
    "spend_monitor_1041.py",
)
STATE_FILE = "/tmp/ads_1041_governor_state.json"
LOG_DIR = os.path.join(
    os.path.expanduser("~"), ".openclaw", "workspace", "logs", "ads_1041_spend_sync.log"
)
...
def parse_click_report(path):
    ...
    return {
        "total": total,
        "by_tag": dict(tag_clicks),
        "by_platform": dict(platform_total),
        "tag_platform": {t: dict(p) for t, p in tag_platform.items()},
    }
...
def parse_commission_report(path):
    ...
    return {
        ...
        "by_tag": {
            t: {"orders": tag_orders[t], "comm": tag_comm[t]} for t in tag_orders
        },
    }
...
def generate_recommendations(clicks, commission):
    ...
    for tag in commission["by_tag"]:
        ...
        if comm > 500000:
            recs["scale"].append(
                {"tag": tag, "comm": comm, "orders": orders, "cvr": cvr}
            )
        elif comm > 50000:
            recs["maintain"].append(
                {"tag": tag, "comm": comm, "orders": orders, "cvr": cvr}
            )
        elif orders > 3:
            recs["observe"].append(
            ...
            recs["kill"].append(
                {"tag": tag, "comm": comm, "orders": orders, "clicks": t_clicks}
            )
...
if __name__ == "__main__":
    ...
    if not args.click or not args.commission:
        print(
            "Usage: python3 sync_1041_report.py --click report.csv --commission report.csv"
        )
        sys.exit(1)
    ...
    log(
        f"✅ Commission report: {commission['total_orders']} orders, Rp {commission['total_comm']:,.0f}"
    )
    ...
    for r in recs["scale"]:
        print(
            f"  🟢 {r['tag']}: Rp {r['comm']:,.0f} ({r['orders']} orders, CVR {r['cvr']:.1f}%)"
        )
    ...
    for r in recs["kill"]:
        ...
        print(
            f"  🔴 {r['tag']}: {r['orders']} orders, Rp {r['comm']:,.0f} from {r['clicks']} clicks{note}"
        )
    ...
    report_path = os.path.join(
        ...
        "workspace",
        "logs",
        f"ads_1041_sync_{__import__('datetime').datetime.now().strftime('%Y%m%d_%H%M%S')}.json",
    )
