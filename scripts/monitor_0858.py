...
LOG_FILE = os.path.join(
    os.path.expanduser("~"), ".openclaw", "workspace", "logs", "ads_0858_monitor.log"
)
...
def pause_hierarchy(cid, cname, reason):
    ...
    check = api(
        f"https://graph.facebook.com/v22.0/{cid}?fields=status&access_token={TOKEN}"
    )
    if check.get("status") != "ACTIVE":
        return

    adsets = api(
        f"https://graph.facebook.com/v22.0/{cid}/adsets?fields=id,name,status&limit=25&access_token={TOKEN}"
    )
    if "error" not in adsets:
        for a in adsets.get("data", []):
            if a.get("status") != "ACTIVE":
                continue
            al = api(
                f"https://graph.facebook.com/v22.0/{a['id']}/ads?fields=id,name,status&limit=25&access_token={TOKEN}"
            )
            if "error" not in al:
                for ad in al.get("data", []):
                    if ad.get("status") == "ACTIVE":
                        api_post(
                            f"https://graph.facebook.com/v22.0/{ad['id']}?access_token={TOKEN}",
                            {"status": "PAUSED"},
                        )
            api_post(
                f"https://graph.facebook.com/v22.0/{a['id']}?access_token={TOKEN}",
                {"status": "PAUSED"},
            )

    r = api_post(
        f"https://graph.facebook.com/v22.0/{cid}?access_token={TOKEN}",
        {"status": "PAUSED"},
    )
    if r.get("success"):
...
def main():
    ...
    # ── Daily spend ──
    ins = api(
        f'https://graph.facebook.com/v22.0/{ACCOUNT}/insights?fields=spend&time_range={{"since":"{today}","until":"{today}"}}&level=account&access_token={TOKEN}'
    )
    daily_spend = (
        float(ins.get("data", [{}])[0].get("spend", 0))
        if "error" not in ins and ins.get("data")
        else 0
    )
    ...
    # ── Get all campaigns ──
    camps = api(
        f"https://graph.facebook.com/v22.0/{ACCOUNT}/campaigns?fields=name,id,status&limit=50&access_token={TOKEN}"
    )
    if "error" in camps:
    ...
    for c in camps.get("data", []):
        ...
        ins2 = api(
            f"https://graph.facebook.com/v22.0/{cid}/insights?fields=spend,impressions,clicks,ctr,cpc&date_preset=today&access_token={TOKEN}"
        )
        d = (
            ins2.get("data", [{}])[0]
            if "error" not in ins2 and ins2.get("data")
            else {}
        )
        ...
        status_icon = (
            "🔴"
            if (cpc > MAX_CPC or (imp >= MIN_IMPRESSIONS and ctr < MIN_CTR))
            else "🟢"
        )
        log(
            f"  {status_icon} {cname[:45]:45s} | Rp{sp:7,.0f} | {imp:5}imp | {clk:4}clk | CPC Rp{cpc:5,.0f} | CTR {ctr:5.2f}%"
        )
