...
def get_account_data(act_id, label, date_preset='yesterday'):
    ...
    camp_insights = api_get(f'{act_id}/insights', {
        'date_preset': date_preset,
...
def format_report(results, date_preset, days_label):
    ...
    period = 'Hari ini' if date_preset == 'today' else 'Kemarin' if date_preset == 'yesterday' else days_label
    
    lines = [f'📋 *ADS DAILY REPORT* — {now}', f'Periode: {period}', '']
    
    grand_total = {'spend': 0, 'impressions': 0, 'clicks': 0, 'purchases': 0, 'leads': 0}
    ...
    for r in results:
        ...
        if tops:
            for t in tops:
                lines.append(f'     → {t["name"][:35]}: Rp{t["spend"]:,.0f} ({t["ctr"]})')
    ...
    if total_purchases:
        lines.append(f'🛒 *Pembelian:* {total_purchases} (CPA Rp{total_spend/total_purchases:,.0f})')
    if total_leads:
