...
if today_spend >= HARDCAP * 0.9:
    ...
    for c in r.json().get('data', []):
        requests.post(f'{BASE}/{c["id"]}', params={'access_token': TOKEN}, json={'status': 'PAUSED'})
        log(f"  🛑 PAUSED: {c['name']}")
    state['paused_by_guard'] = True
    state['paused_date'] = today

# Check individual campaign CPC/CTR
r2 = requests.get(f'{BASE}/{ACT}/insights', params={
    'access_token': TOKEN,
...
for camp in r2.json().get('data', []):
    ...
    if cpc > CPC_LIMIT and camp_id:
        ...
        requests.post(f'{BASE}/{camp_id}', params={'access_token': TOKEN}, json={'status': 'PAUSED'})
...
if state.get('paused_by_guard') and paused_date != today:
    ...
    r = requests.get(f'{BASE}/{ACT}/campaigns', params={
        'access_token': TOKEN, 'fields': 'id,name', 'limit': 50,
        'filtering': json.dumps([{'field':'campaign.effective_status','operator':'IN','value':['PAUSED']}])
    })
    for c in r.json().get('data', []):
        if 'V2_HV' in c.get('name', ''):
            requests.post(f'{BASE}/{c["id"]}', params={'access_token': TOKEN}, json={'status': 'ACTIVE'})
