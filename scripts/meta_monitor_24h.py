...
def monitor_and_execute():
    ...
    try:
        ...
        for camp in camps:
            ...
            ins_r = requests.get(f'https://graph.facebook.com/v19.0/{cid}/insights', 
                                 params={'access_token': ACCESS_TOKEN, 'date_preset': 'last_3d', 'fields': 'spend,inline_link_clicks,inline_link_click_ctr'}).json()
            ins_data = ins_r.get('data', [])
            
            if not ins_data or status != 'ACTIVE': continue
            ...
            if 'rak dapur' in name.lower() and ctr > 7.0 and cpr < 120 and budget < 1000000:
                new_budget = int(budget * 1.5)
                requests.post(f'https://graph.facebook.com/v19.0/{cid}', params={'daily_budget': new_budget, 'access_token': ACCESS_TOKEN})
                log_entries.append(f'SCALED (Trend): {name} ({budget}->{new_budget}) CTR:{ctr:.2f}% CPR:{cpr:.0f}')

            # 2. KILL LOSERS (CPR > 180 - slightly looser threshold because of lag)
            elif cpr > 180 and spend > 50000:
                requests.post(f'https://graph.facebook.com/v19.0/{cid}', params={'status': 'PAUSED', 'access_token': ACCESS_TOKEN})
