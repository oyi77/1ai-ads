...
# File paths
EXCEL_ADS = os.path.join(
    os.path.expanduser("~"),
    ".openclaw",
    "media",
    "inbound",
    "Selow-ID-0858-Kampanye-13-Mei-2026-13-Mei-2026_1---5be73abf-d775-4970-8ff9-725908cc8ed6.xlsx",
)
CLICK_CSV = os.path.join(
    os.path.expanduser("~"),
    ".openclaw",
    "media",
    "inbound",
    "WebsiteClickReport202605141242---e98a09e4-6966-49f1-9519-fd16fefe37c2.csv",
)
COMMISSION_CSV = os.path.join(
    os.path.expanduser("~"),
    ".openclaw",
    "media",
    "inbound",
    "AffiliateCommissionReport202605141242---ee139e64-585f-4131-8065-49faf7dc98ba.csv",
)
...
def analyze_0858():
    ...
    df_ads["spend"] = pd.to_numeric(
        df_ads["Jumlah yang dibelanjakan (IDR)"], errors="coerce"
    ).fillna(0)
    ...
    for _, row in df_ads.iterrows():
        ...
        spend = row["spend"]
        tag = "unknown"
        if "rakpiringpengering" in name.lower():
            tag = "rakpiringpengering"
        elif "dongkrakelektrik" in name.lower():
            tag = "Dongkrakelektrik"
        elif "gendongananjing" in name.lower():
            tag = "gendongananjing"
        elif "testing_rak" in name.lower():
            tag = "rakpiringpengering"  # Usually same niche
    ...
    df_comm["earnings"] = pd.to_numeric(
        df_comm["Komisi Bersih Affiliate (Rp)"], errors="coerce"
    ).fillna(0)
    agg_comm = (
        df_comm.groupby("tag")
        .agg(orders=("ID Pemesanan", "nunique"), total_comm=("earnings", "sum"))
        .reset_index()
    )
    ...
    summary["ROI%"] = (
        (summary["profit"] / summary["cost_plus_tax"] * 100)
    ...
    print(
        summary[
            ["tag", "spend", "cost_plus_tax", "total_comm", "profit", "ROI%"]
    ...
    for tag in summary["tag"].unique():
        ...
        print(f"\n[TAG: {tag}]")
        prods = (
            df_comm[df_comm["tag"] == tag]
            .groupby("Nama Barange")["earnings"]
            .sum()
