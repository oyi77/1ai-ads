            ...
            if not halted_early:
                ...
                for pos in active_pos:
                    if self._position_held_too_long(pos):
                        print(
                            f"  ⏰ Max hold time exceeded. Closing {pos['symbol']}..."
                        )
                ...
                active_pos = [
                    ...
                    and p.get("symbol") not in active_symbols
                ]
                active_pos = active_pos or [
                    p
                    for p in self.check_open_positions()
                    if float(p.get("contracts", 0)) > 0
                ]
                active_symbols = {p["symbol"] for p in active_pos}
                ...
                if self.session != "off_session" and len(active_pos) < 3:
                    ...
                    for c in candidates:
                        ...
                        if (
                            long_count >= MAX_SAME_DIRECTION_POSITIONS
                            and c.get("change_24h", 0) < -1
                        ):
                            print(
                            ...
                            continue
                        if (
                            short_count >= MAX_SAME_DIRECTION_POSITIONS
                            and c.get("change_24h", 0) > 1
                        ):
                            print(
