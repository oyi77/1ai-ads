...
TOKEN = subprocess.run(
    ["grep", "-oP", "ACCESS_TOKEN = '***']+", str(script_dir / "list_ad_accounts.py")],
    capture_output=True,
    text=True,
).stdout.strip()
...
# ═══ 1. MONITOR STATUS ═══
r = subprocess.run(
    ["systemctl", "is-active", "vilona-ads0858.service"], capture_output=True, text=True
).stdout.strip()
...
print(f"   [{bar}] {pct:.0f}%")
print(
    f"   👆 {clicks} clicks | 👁️ {impressions:,} imp | 💵 CPC Rp {cpc:,.0f} | 📊 CTR {ctr:.2f}%"
)

# ═══ 3. ACTIVE CAMPAIGNS ═══
camps = api(
    f"{BASE}/{ACT}/campaigns?fields=name,id,status,effective_status&limit=50&access_token=***"
)
...
for c in active:
    cid = c["id"]
    ins2 = api(
        f"{BASE}/{cid}/insights?fields=spend,clicks,impressions,cpc,ctr&date_preset=today&access_token=***"
    )
    ...
    icon = (
        "🔴"
        if cp > 150
        else ("🔴" if (ct < 3 and int(d2.get("impressions", 0)) >= 500) else "🟢")
    )
    print(
        f"  {icon} {c['name'][:42]:42s} | Rp{sp:7,.0f} | CPC {cp:5.0f} | CTR {ct:5.2f}% | {cl:4} clk"
    )
...
alerts = []
if r != "active":
    alerts.append("🔴 Monitor v5 is DOWN!")
if spend >= 300000:
    alerts.append("🔴 BUDGET CAP 300rb TERCAPAI!")
elif spend >= 270000:
    alerts.append("🟡 Budget 90%+ (Rp {:.0f})".format(spend))
if cpc > 150 and clicks > 10:
    alerts.append(f"🔴 CPC Rp {cpc:,.0f} > 150!")
if ctr < 3 and impressions > 500:
    alerts.append(f"🔴 CTR {ctr:.2f}% < 3%!")
if not active:
    alerts.append("🟡 NO ACTIVE CAMPAIGNS!")
