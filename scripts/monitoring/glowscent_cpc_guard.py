#!/usr/bin/env python3
"""GlowScent CPC Guardian — 15-min interval monitor for act_2125021885010866"""
import requests, json, time, os, sys
from datetime import datetime
from pathlib import Path

# === CONFIG ===
ACCOUNT = 'act_2125021885010866'
TEST_CPC_LIMIT = 200
WINNER_CPC_LIMIT = 150
MIN_CLICKS_FOR_JUDGMENT = 3  # need at least 3 clicks to judge CPC
LOG_FILE = Path(__file__).parent.parent / 'logs' / 'glowscent_cpc_guard.log'
...
log("=" * 60)
log("GLOWSCENT CPC GUARDIAN STARTED")
...
insights = api(f'{ACCOUNT}/insights', {
    ...
    'time_range': json.dumps({'since': datetime.now().strftime('%Y-%m-%d'), 'until': datetime.now().strftime('%Y-%m-%d')}),
})
...
# 2. Check for OFF_ campaigns that are still running
for cid, c in camp_status.items():
    name_lower = c.get('name', '').lower()
    if name_lower.startswith('off_') and c.get('effective_status') == 'ACTIVE':
        log(f"🚨 WARNING: OFF_ campaign STILL ACTIVE: {c['name']} {cid}")
