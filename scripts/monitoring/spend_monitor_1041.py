...
DEAD_TAGS = [
    ...
    "BKlaundry2",
    "BKlaundry3",
    "herborist-reels-selingkuhituindah-fb",
    "Tiraimanik",
    "toiletmobil",
    "icecubemaker",
    "rotatingstoragebox",
...
def cpc_patrol():
    ...
    if total > 0:
        log(
            f"🛡️ PATROL V7: {len(paused_cpc)} CPC>150 + {len(paused_ctr)} CTR<3% + {len(paused_zero)} 0klik + {len(paused_label)} label"
        )
...
def get_today_spend():
    ...
    try:
        data = api(url)
        return (
            float(data.get("data", [{}])[0].get("spend", 0)) if data.get("data") else 0
        )
    except Exception:
        return 0
...
def get_yesterday_roi():
    ...
    try:
        ...
        if data and len(data) > 0:
            ...
            if sp > 0 and comm > 0:
                roi = comm / sp
                log(
                    f"📊 Yesterday ROI (Supabase): {roi:.2f}x (Comm Rp{comm:,.0f} / Spend Rp{sp:,.0f})"
                )
                return roi, sp
    ...
    try:
        ...
        if spend > 0:
            log(
                f"⚠️ ROI fallback (FB only, no commission): assume 1.0x | Spend Rp{spend:,.0f}"
            )
            return 1.0, spend
        return 1.0, 0
    except Exception:
...
def main():
    ...
    if hour == RUN_START and not state.get("resumed_today"):
        ...
        log(
            f"🎯 GAS-REM: {action} → Rp {new_budget:,}/hari (yesterday ROI {yesterday_roi:.2f}x, spend Rp{yesterday_spend:,.0f})"
        )
    ...
    if (
        today_spend >= soft_threshold
        and not state.get("paused_today")
        and not state.get("soft_paused")
    ):
    ...
    log(
        f"   Spend:   Rp {today_spend:,.0f} ({today_spend/daily_budget*100:.0f}%)"
        if daily_budget > 0
        else f"   Spend:   Rp {today_spend:,.0f}"
    )
    log(f"   Sisa:    Rp {max(0, daily_budget - today_spend):,}")
    log(
        f"   Status:  {'⏸️ PAUSED' if state.get('paused_today') else '💛 SOFT' if state.get('soft_paused') else '▶️ RUNNING'}"
    )
...
if __name__ == "__main__":
    if "--daemon" in sys.argv:
        ...
        log(
            f"📋 Rules: HardCap Rp{HARD_CAP:,} | SoftCap {int(SOFT_CAP_PCT*100)}% | CPC KPI Rp{CPC_KPI} | GAS>{ROI_GAS}x | REM<{ROI_REM}x"
        )
        while RUNNING:
            try:
                main()
