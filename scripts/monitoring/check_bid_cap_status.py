...
def check_bid_cap_status():
    ...
    c_res = requests.get(
        ...
        params={
            ...
            "fields": "name,status,effective_status,bid_strategy",
        },
    ).json()
    print(
        f"Campaign: {c_res.get('name')} | Status: {c_res.get('effective_status')} | Strategy: {c_res.get('bid_strategy')}"
    )
    ...
    as_res = (
        requests.get(
            ...
            params={
                "access_token": ACCESS_TOKEN,
                "fields": "name,status,effective_status,bid_amount",
            },
        )
        .json()
        .get("data", [])
    ...
    for a in as_res:
        print(
            f"  AdSet: {a.get('name')} | Status: {a.get('effective_status')} | Bid Cap: {a.get('bid_amount')}"
        )
