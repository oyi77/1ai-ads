...
clones_to_kill = [
    (
        "120245814356570121",
        "VILONA_AUTO_CBO_Rakdapur3_Movies _CLONE_VILONA",
        "DEAD_NoSpend",
    ),
...
for cid, cname, reason in clones_to_kill:
    result = api_post(
        cid, {"name": f'{reason}_{cname.replace(" ","_")}', "status": "PAUSED"}
    )
...
keepers = [
    (
        "120245814350950121",
        "VILONA_AUTO_CBO_Rakdapur3_Movies_CLONE_VILONA",
        "CBO_Rakdapur3_Movies_WinnerA",
    ),
    (
        "120245688589920121",
        "VILONA_AUTO_CBO_Rakdapur3_Movies (4)",
        "CBO_Rakdapur3_Movies_WinnerB",
    ),
]
print("\n=== RENAMING KEEPERS ===")
...
for cid, cname in cpc_high:
    result = api_post(
        cid, {"name": f'DEAD_CPC169_{cname.replace(" ","_")}', "status": "PAUSED"}
    )
