...
def fix_adsets(token):
    ...
    for a in needs_fix:
        ...
        geo = {
            "cities": [{"key": c["key"]} for c in CITIES],
        ...
        if r2.json().get("success"):
            ...
            log(
                f"  ❌ {a['name']}: {err.get('error_user_msg', err.get('message',''))[:60]}"
            )
...
def mark_profitable(token):
    r = requests.get(
        f"https://graph.facebook.com/v19.0/{AD_ACCT}/insights",
        params={
            "access_token": token,
    ...
    for c in campaigns:
        ...
        if cpc < 130 and ctr > 4.0 and clicks > 50 and "[WINNER]" not in name:
            time.sleep(5)
            r2 = requests.post(
                f"https://graph.facebook.com/v19.0/{camp_id}",
                params={"access_token": token, "name": f"[WINNER] {name}"},
                timeout=30,
            )
            if r2.json().get("success"):
                winners += 1
...
def main():
    ...
    while attempt < 60:
        ...
        if test_rate_limit(token):
            ...
            break
        log(
            f"⏳ Attempt {attempt}: rate limited, attempt {attempt}/{max_attempts}, waiting {wait}s..."
        )
