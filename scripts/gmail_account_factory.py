...
def run_js(ws, js_code):
    ...
    msg = json.dumps({"id": 1, "method": "Runtime.evaluate", "params": {"expression": js_code}})
    ws.send(msg)
...
def login_gmail(ws, email, password):
    ...
    # Navigate to Gmail
    run_js(ws, "window.location.href = 'https://accounts.google.com/signin/v2/identifier?service=mail'")
    time.sleep(4)
    ...
    if success:
        ...
        for platform in ["instagram", "twitter", "youtube"]:
            try:
                ...
                result['accounts_created'][platform] = {"success": False, "error": str(e)[:80]}
...
def check_all_gmails():
    ...
    for i, acct in enumerate(accounts[:10]):  # Test first 10
        ...
        try:
            ...
            results.append({"email": acct['Email'], "success": False, "error": str(e)[:100]})
            failed += 1
