...
def generate_report(reddit_hot, reddit_search, inbox, twitter_results=None):
    ...
    if unique_posts:
        ...
        for p in unique_posts[:10]:
            ...
            lines.append(
                f"- by u/{p.get('author', '?')} | {p.get('comments', 0)} comments"
            )
            if p.get("url"):
    ...
    if unique_posts:
        ...
        lines.append(
            f"1. **Engage:** Comment on [{best_post.get('title', '')}]({best_post.get('url', '')}) — trending in r/{best_post.get('subreddit', '')}"
        )
        lines.append(
            f"   *Suggested angle:* Share your experience with {best_post.get('matched_keyword', 'similar topics')}"
        )
        lines.append("")
    ...
    if (
        "qwen"
        in " ".join(str(p.get("matched_keyword", "")) for p in unique_posts).lower()
    ):
    ...
    lines.append(f"---")
    lines.append(
        f"*Next scan in ~{SCAN_INTERVAL_HOURS}h. Automatically generated by Brand Watch v1.*"
    )
