...
def check_0858():
    ...
    for camp in camp_list:
        ...
        if is_blacklisted(cname) and status == "ACTIVE":
            ...
            if resp.get("success"):
                msg = (
                    f"🚫 BLACKLIST ENFORCED: {cname[:55]} — auto-paused (negative ROI)"
                )
                alerts.append(msg)
                actions.append(msg)
    ...
    active_camps = [
        c for c in camps.get("data", []) if c.get("effective_status") == "ACTIVE"
    ]
    ...
    for camp in active_camps:
        ...
        cname = camp.get("name", "?")
        budget = (
            int(camp.get("daily_budget", 0)) / 100 if camp.get("daily_budget") else 0
        )
        ...
        if spend_7d == 0 and imp_7d == 0:
            ...
            if first_seen:
                ...
                if hours_active > 24:
                    msg = (
                        f"⚠️ ZERO DELIVERY >24h: {cname[:50]} — active tapi gak tayang"
                    )
                    alerts.append(msg)
        ...
        if ctr_val >= 4.0 and spend_7d > 0:
            ...
            if last_scale:
                try:
                    days_since = (
                        datetime.now() - datetime.fromisoformat(last_scale)
                    ).days
    ...
    for camp in active_camps:
        ...
        for adset in adsets.get("data", []):
            ...
            if bid_strategy == "LOWEST_COST_WITH_BID_CAP":
                expected_bid = (
                    BID_CAP_KAKRIPUT if "Kakriput" in cname else BID_CAP_DEFAULT
                )
            ...
            if len(platforms) > 1 and set(platforms).intersection(
                {"facebook", "instagram"}
            ):
            ...
            ads = api_get(
                f"{aid}/ads", {"fields": "id,name,effective_status", "limit": 25}
            )
    ...
    try:
        ...
        subprocess.run(
            [
                ...
                "add",
                "--title",
                f'0858 Auto-Check {datetime.now().strftime("%Y-%m-%d %H:%M")}',
                "--type",
                "concept",
                "--content",
                report_text,
