...
BASE = Path(os.path.join(os.path.expanduser("~"), ".openclaw", "workspace"))
LOG_DIR = BASE / "logs"
...
class CampaignOptimizer:
    ...
    def generate_report(self):
        ...
        lines.append(f"   Target: Min ROI 2x | Updated: {datetime.now().strftime('%H:%M')}")
        lines.append(f"{'='*60}")
        lines.append(f"{'Tag':<25} {'Clicks':<8} {'Ord':<5} {'CVR':<8} {'Komisi':<12} {'Est.ROI':<8} {'Action':<12}")
        lines.append(f"{'-'*60}")
        
        for tag, data in sorted(self.last_analysis.items(), key=lambda x: -x[1]['komisi']):
            roi = data['est_roi_pending']
            roi_str = f"{roi:.1f}x" if roi > 0 else "0x"
            act = data['action']
            lines.append(f"{tag:<25} {data['clicks']:<8} {data['orders']:<5} {data['cvr']:<8} Rp {data['komisi']:<8,.0f} {roi_str:<8} {act:<12}")
        ...
        lines.append(f"  TARGET MIN: 2.0x | STATUS: {'✅ PROFIT' if overall_roi >= 2 else '⚠️ BELOW TARGET'}")
        
        # Recommendations
        stop_tags = [t for t, d in self.last_analysis.items() if 'STOP' in d['action']]
        increase_tags = [t for t, d in self.last_analysis.items() if 'INCREASE' in d['action']]
...
if __name__ == "__main__":
    ...
    if len(sys.argv) >= 3:
        ...
        print("Usage: python3 campaign_optimizer.py <account_id> <commission.csv> <clicks.csv>")
