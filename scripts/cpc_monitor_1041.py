...
def main():
    ...
    campaigns = api_get(f'{ACT_ID}/campaigns', {
            ...
            if impr > 0:
                if cpc > CPC_LIMIT:
                    ...
                    if result.get('success'):
                        paused_any = True
                        log(f"🛑 PAUSED: {cname:50s} | CPC {cpc:.0f} > {CPC_LIMIT} | Strike {death_count}/{DEATH_PENALTY}")
                    else:
                        log(f"❌ FAILED: {cname} - {result}")
                elif cpc > 0:
                    log(f"  ✅ {cname:50s} | Spend: {spend:>7,} | CPC: {cpc:>5.0f} | CTR: {ctr:>5.2f}%")
    ...
    for cid, pause_info in list(state.get('paused', {}).items()):
        ...
        if cpc <= REACTIVATE_GAS_CPC and ctr >= CTR_MIN_GAS:
            # GAS — reactive + scale
            orig_name = pause_info.get('original_name', cname.replace('PAUSED_', '').replace('DEAD_', ''))
            result = api_post(cid, {
                'name': f'REACTIVATED_{orig_name}',
                'status': 'ACTIVE'
            })
            if result.get('success'):
                del state['paused'][cid]
                log(f"🟢 GAS REACTIVATE: {cname:50s} | CPC {cpc:.0f} | CTR {ctr:.2f}% | Strike {death_count}")
            ...
            if result.get('success'):
                del state['paused'][cid]
                log(f"🟡 REACTIVATE: {cname:50s} | CPC {cpc:.0f} | CTR {ctr:.2f}% | Strike {death_count}")
            ...
            if result.get('success'):
                del state['paused'][cid]
                log(f"⚠️ CAUTIOUS REACTIVATE: {cname:50s} | CPC {cpc:.0f} | CTR {ctr:.2f}% | Strike {death_count}")
            ...
            if death_count_entry >= DEATH_PENALTY:
                ...
                if result.get('success'):
                    del state['paused'][cid]
                    log(f"💀 DEAD CONFIRMED: {cname:50s} | CPC {cpc:.0f} still > {DEAD_CONFIRMED_CPC} after {COOLING_MINUTES}m")
