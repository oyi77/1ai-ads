...
def rule_scale_budget(cid, current_budget, cpa):
    ...
    if cpa <= RULES["cpr"]["target"]:
        ...
        log(
            f'   CPA Rp{cpa:,} ≤ Target Rp{RULES["cpr"]["target"]:,} → SCALE UP ke Rp{scale:,}'
        )
        ...
        log(
            f'   CPA Rp{cpa:,} masih dalam batas wajar (max Rp{RULES["cpr"]["max_acceptable"]:,})'
        )
        ...
        log(
            f'   ⚠️ CPA Rp{cpa:,} > Max Rp{RULES["cpr"]["max_acceptable"]:,} → TURUN ke Rp{down:,}'
        )
...
def rule_cpr_monitor(cid, aid, current_cpa, active_adset={}):
    ...
    if not bid_strategy or bid_strategy != "LOWEST_COST_WITH_BID_CAP":
        ...
        if "success" in r:
            log(
                f'   ✅ Bid strategy = LOWEST_COST_WITH_BID_CAP | Bid cap = Rp{RULES["cpr"]["bid_cap"]:,}'
            )
        else:
            log(
                f'   ⚠️ Gagal set bid strategy: {r.get("error",{}).get("message","?")[:80]}'
            )

    if current_cpa < RULES["cpr"]["target"] * 0.7:
        new_bid = min(int(RULES["cpr"]["bid_cap"] * 1.2), 50000)
        log(
            f"   💪 CPA Rp{current_cpa:,} < 70% target → scale bid cap ke Rp{new_bid:,}"
        )
...
def rule_duplicate_to_fb(cid, days_running):
    ...
    if days_running < RULES["duplicate"]["days_stable"]:
        log(
            f'⏳ RULE 4: Baru {days_running} hari, tunggu {RULES["duplicate"]["days_stable"]} hari'
        )
        return
...
def run_monitor(campaign_id=WINNING_CONFIG["campaign_id"]):
    ...
    adsets = fb_get(
        ...
        {"fields": "id,name,effective_status,bid_amount,bid_strategy", "limit": 5},
    )
    ...
    # Get ad in adset
    ads = fb_get(f"{aid}/ads", {"fields": "id,name,effective_status", "limit": 5})
    active_ad = None
...
def show_rules():
    ...
    print(
        f'   Obj: {WINNING_CONFIG["objective"]} → OFFSITE_CONVERSIONS ({WINNING_CONFIG["conversion_event"]})'
    )
    print(
        f'   Platform: {WINNING_CONFIG["platform"]} | Age: {WINNING_CONFIG["age_min"]}-{WINNING_CONFIG["age_max"]}'
    )
    print(
        f'   Interest: {WINNING_CONFIG["interest"]} | Bid: Rp{WINNING_CONFIG["bid_cap"]:,}'
    )
    print(
        f'   HPP: Rp{WINNING_CONFIG["hpp"]:,} | Jual: Rp{WINNING_CONFIG["harga_jual"]:,}'
    )
    print(
        f'   Target CPR: Rp{WINNING_CONFIG["cpr_target"]:,} | Max: Rp{WINNING_CONFIG["cpr_max"]:,}'
    )
