...
completed_orders = pd.read_csv(
    os.path.join(
        ...
        "media",
        "inbound",
        "AffiliateCommissionReport202605061348---58f678eb-edec-40f5-b45c-f74476a21e49.csv",
    )
)
...
instagram_orders = next(
    (p for p in platform_data if p["Platform"] == "Instagram"), None
)
...
if others_total > 0:
    print(
        f"  Others: 15% (split among: {', '.join([p['Platform'] for p in others_orders])})"
    )
else:
...
for i, product in enumerate(top_5, 1):
    ...
    print(
        f"  {i}. {product['Nama Barange'][:45]:45s} - Rp {product_revenue:,} ({pct:.2f}%)"
    )
...
print(
    f"  Top 5 Products: Rp {recommended_top5:,} ({recommended_top5/total_revenue*100:.1f}%)"
)
print(
    f"  Test Budget: Rp {recommended_test:,} ({recommended_test/total_revenue*100:.1f}%)"
)
print(
    f"  Monitor Budget: Rp {recommended_monitor:,} ({recommended_monitor/total_revenue*100:.1f}%)"
)
...
scale_plan = {
    "platform_budget": {
        "Instagram": 85,
        "Others": (
            sum(p["Orders"] for p in others_orders)
            / sum(p["Orders"] for p in platform_data)
            * 100
            if sum(p["Orders"] for p in platform_data) > 0
            else 0
        ),
    },
    "product_budget": {
