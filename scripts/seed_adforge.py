...
templates = [
    # F&B / Kuliner
    ('template_fnb_1', 'food', 'F&B Pain → Solution', 'Bosan [pain]? Coba [product], beda dari yang lain.',
     '{pain} bikin [target] gak nyaman? {product} hadir dengan {benefit}. Klik di bio!', 'Pesan Sekarang - Stok Terbatas ⏳'),
    ('template_fnb_2', 'food', 'F&B Limited Offer', 'Promo [discount] untuk [target] — hari ini doang!',
     'Dapetin {product} dengan harga spesial cuma hari ini. {benefit} cocok buat {target}.', 'Beli Sekarang — Diskon {discount}'),
    
    # Fashion
    ('template_fashion_1', 'fashion', 'Fashion Before-After', 'Ini [target] sebelum pakai [product] vs sesudahnya. Gaslight?',
     '{product}: {benefit}. {pain} langsung ilang. Review {rating}⭐ dari {count}+ pembeli.', 'Pesen Sekarang — Gratis Ongkir 🚚'),
    ('template_fashion_2', 'fashion', 'Fashion Trend', 'Tren [year] udah keluar! [target] wajib punya.',
     'Jangan ketinggalan! {product}, {benefit}. #{target} #{vibe}.', 'Cek Koleksi Lengkap →'),
    
    # Home & Living
    ('template_home_1', 'home', 'Home Problem Solver', 'Rapihin [area] cuma 5 menit pake [product]!',
     'Gak perlu ribet. {product} bikin {area} lo rapi dalam hitungan detik. {benefit}.',
     'Beli Sekarang — Rp{price}'),
    ('template_home_2', 'home', 'Home Efficiency', 'Buang [pain] selamanya — solusi untuk [target].',
     '{product} adalah jawaban buat {target}. {benefit}. Udah dipakai {count}+ rumah tangga.',
     'Gas — Pesan di sini ✅'),
    
    # Health & Beauty
    ('template_beauty_1', 'beauty', 'Beauty Results', 'Hasil setelah [days] hari pake [product] — no filter.',
     'Beneran works! {product} bantu {target} dapetin {benefit}. Review {rating}⭐ bukti nyata.',
     'Order Sekarang — Dijamin Original ✅'),
    
    # Electronics
    ('template_elec_1', 'electronics', 'Electronics Upgrade', 'Lo masih pake [old]? Udah saatnya upgrade ke [product].',
     '{product}: {benefit}. Lebih {compare} dibanding [old]. Garansi [warranty].',
     'Cek Spek Lengkap →'),
    
    # Automotive
    ('template_auto_1', 'automotive', 'Auto Accessory', 'Pasang [product] di [vehicle] — hasilnya gila.',
...
rules = [
    ...
    ('rule_spend_kill_1', None, 'Spend No Conv', 'conversions', '==', 0, 'alert_owner', 200000),
    ('rule_cpa_scale_1', None, 'CPA Good - Scale', 'cpa', '<', 5000, 'scale_budget_20', 1),
    ('rule_roas_scale_1', None, 'ROAS Good - Scale', 'roas', '>', 2, 'scale_budget_30', 1),
    ('rule_budget_kill_1', None, 'Budget Exhausted', 'spend', '>', 500000, 'alert_owner', 0),
]

for r in rules:
    rid, cid, name, metric, op, val, action, act_val = r
    c.execute('''INSERT OR IGNORE INTO automation_rules
...
sample_ads = [
    ('ad_rakdapur_1', 'Rak Piring Pengering Minimalis', 'Rak Piring', 
     'Ibu rumah tangga', 'Anti karat, tahan lama, muat banyak piring',
     ...
     'Pesan Sekarang 🔥'),
    ('ad_dongkrak_1', 'Dongkrak Elektrik Portable', 'Dongkrak Elektrik',
     'Pengendara mobil', 'Mudah dipake, angkat mobil 3 ton, portabel',
    ...
    ('ad_organizer_1', 'Organizer Pullout Magic', 'Organizer Pullout',
     'Hunian minimalis', 'Multi fungsi, gak perlu bor, muat banyak',
...
for a in sample_ads:
    ...
    c.execute('''INSERT OR IGNORE INTO ads
...
pages = [
    ('lp_rakdapur', 'Rak Piring Pengering', 'hero_product', 'Rak Piring Pengering Minimalis',
     89000, '["Piring berantakan", "Dapur becek", "Gak ada tempat jemur"]',
     '["Kering otomatis tanpa lap", "Muati 30+ piring", "Anti karat", "Tanpa bor - tempel doang"]',
     'Pesan Sekarang — Diskon 30% ⏳', 'Lihat Testimoni', 'https://lynk.id/jendralbot',
     'pesanan-di-handler'),
    ('lp_dongkrak', 'Dongkrak Elektrik Portable', 'hero_product', 'Dongkrak Elektrik Otopal',
     150000, '["Bocor ban di jalan", "Dongkrak manual berat", "Takut diketokin"]',
     '["Angkat 3 ton", "Listrik portabel", "5 menit jadi", "Garansi 1 tahun"]',
     'Beli Sekarang', 'Cek Review', 'https://lynk.id/jendralbot',
     'pesanan-di-handler'),
]

for p in pages:
    pid, name, template, product, price, pains, benefits, cta1, cta2, wa, checkout = p
    c.execute('''INSERT OR IGNORE INTO landing_pages
        ...
        VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, 'published', ?)''',
        (pid, name, template, product, price, json.dumps(pains), json.dumps(benefits), cta1, cta2, wa, checkout, now))
    ...
    if os.path.exists(fp):
        with open(fp) as lf:
            ...
            for i, line in enumerate(lines[-5:]):
                ...
                c.execute('''INSERT OR IGNORE INTO competitor_snapshots
                    (id, url, platform, ad_data, snapshot_type, captured_at)
                    VALUES (?, ?, ?, ?, 'auto', ?)''',
                    (sid, f'https://www.facebook.com/ads/library/?q={i}', 'meta', 
                     json.dumps({'raw_log': line.strip(), 'source': 'adslib_monitor'}), now))
