...
SESSION_PATH = os.path.expanduser("~/.openclaw/workspace/.vilona/sessions/veris_oc_monitor.session")
...
def build_report(sessions, check_num):
    ...
    unknown = [s for s in sessions if s.get("status") not in ("running", "done", "failed", "error")]
...
async def main_loop():
    ...
    while True:
        try:
            ...
            log(f"📤 Report #{check_num} sent to Report Army Group ({len(sessions)} sessions)")
