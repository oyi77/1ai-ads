...
TOKEN = subprocess.run(["grep", "-oP", "ACCESS_TOKEN = '\\K[^']+", 
...
r = requests.get(f'{BASE}/{ACT}/insights', params={
    'access_token': TOKEN,
...
log(f"Yesterday: Spend Rp {total_spend:,.0f} | Clicks {total_clicks} | Impr {total_impressions}")
...
for camp in yesterday_data:
    ...
    if cpc < 150 and ctr > 3 and spend > 5000:
        winning_camps.append({
            ...
            'spend': spend, 'cpc': cpc, 'ctr': ctr, 'clicks': clicks
        })
...
for camp in winning_camps:
    ...
    r = requests.get(f'{BASE}/{camp["id"]}/adsets', params={
        'access_token': TOKEN, 'fields': 'id,name,daily_budget',
        'limit': 10,
    })
    for adset in r.json().get('data', []):
        ...
        if current_budget > 0:
            ...
            if new_budget > current_budget:
                requests.post(f'{BASE}/{adset["id"]}', params={'access_token': TOKEN}, 
                            json={'daily_budget': new_budget})
                log(f"  📈 Scaled {camp['name']}: Rp {current_budget:,} → Rp {new_budget:,}")
...
creatives = {'rakpiringpengering': '2218448638921797', 'organizerpullout': '1811776943540814'}
...
r = requests.get(f'{BASE}/{ACT}/campaigns', params={
...
for camp in winning_camps[:2]:
    ...
    product = camp['name'].split('_')[2] if len(camp['name'].split('_')) > 2 else 'rakpiringpengering'
    ...
    for theme_name, interests in interest_pool.items():
        ...
        # Campaign
        r = requests.post(f'{BASE}/{ACT}/campaigns', params={'access_token': TOKEN}, json={
            'name': new_name, 'objective': 'OUTCOME_TRAFFIC', 'status': 'ACTIVE',
        ...
        r2 = requests.post(f'{BASE}/{ACT}/adsets', params={'access_token': TOKEN}, json={
            'name': f"AdSet_SCALE_{product}_{theme_name}",
            'campaign_id': camp_id, 'status': 'ACTIVE', 'daily_budget': 20000,
            'billing_event': 'IMPRESSIONS', 'optimization_goal': 'LINK_CLICKS',
            'bid_strategy': 'LOWEST_COST_WITH_BID_CAP', 'bid_amount': 150,
            'targeting': t,
        })
        adset_res = r2.json()
        ...
        # Ad
        r3 = requests.post(f'{BASE}/{ACT}/ads', params={'access_token': TOKEN}, json={
