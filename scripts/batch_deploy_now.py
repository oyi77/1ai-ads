...
r = requests.get(
    ...
    params={"access_token": cfg["meta"]["access_token"], "limit": 15},
).json()
pages = r["data"][:10]
    ...
    for p in pages:
        try:
            r = requests.post(
                ...
                params={
                    "message": message.format(link=LYNK),
            ...
            if "id" in r:
                print(
                    f'✅ [{now}] {p["name"]} — [{post_id}] https://facebook.com/{r["id"]}'
                )
                results.append(
                    {
                        "time": now,
            ...
            print(
                f'⚠️ [{datetime.now().strftime("%H:%M:%S")}] {p["name"]} — EXCEPTION: {str(e)[:100]}'
            )
        sys.stdout.flush()
        time.sleep(3)
