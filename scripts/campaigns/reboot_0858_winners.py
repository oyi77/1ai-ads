...
def reboot():
    ...
    for cid in WINNER_IDS:
        ...
        r = requests.post(
            url, data={"access_token": ACCESS_TOKEN, "status": "ACTIVE"}
        ).json()
        if r.get("success"):
    ...
    adsets = (
        requests.get(
            as_url, params={"access_token": ACCESS_TOKEN, "fields": "id,status"}
        )
        .json()
        .get("data", [])
    )
    for aset in adsets:
        requests.post(
            f'https://graph.facebook.com/v19.0/{aset["id"]}',
            data={"access_token": ACCESS_TOKEN, "status": "ACTIVE"},
        )
