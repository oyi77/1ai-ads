...
_token_path = os.path.join(
    os.path.dirname(os.path.abspath(__file__)), "spend_monitor_1041.py"
)
try:
...
def load_shopee_data(date_str=None):
    ...
    click_files = sorted(
        ...
        key=lambda f: os.path.getsize(os.path.join(MEDIA_DIR, f)),
        reverse=True,
    )
    order_files = sorted(
    ...
    if click_files:
        ...
        print(
            f"   📂 Clicks: {click_files[0][:50]}... ({os.path.getsize(fpath):,} bytes)"
        )
    ...
    if order_files:
        ...
        print(
            f"   📂 Orders: {order_files[0][:50]}... ({os.path.getsize(fpath):,} bytes)"
        )
        with open(fpath, encoding="utf-8-sig") as f:
...
def match_campaigns_to_shopee(campaigns, orders):
    ...
    rd3_total_spend = sum(
        ...
        if c["tag"] == "rakdapur3" and c["spend"] > 0
    )
...
def generate_report(campaigns, days=1):
    ...
    date_str = (
        ...
        if days == 1
        else f"{(now-timedelta(days=days)).strftime('%d %b')} - {now.strftime('%d %b %Y')}"
    )
    ...
    lines.append(
        f"  📈 TOTAL PROFIT:   Rp {total_profit:>12,.0f}  ({'✅ UNTUNG' if total_profit > 0 else '❌ RUGI'})"
    )
    lines.append(f"  🎯 ROI KESELURUHAN: {total_roi:.2f}x")
    lines.append("")
    lines.append(
        f"  🔥 EFISIEN: {efisien}  |  ✅ JALAN: {jalan}  |  ⚠️ PANTAU: {pantau}  |  💀 BUSUK: {busuk}  |  ⏳ NO SPEND: {nospend}"
    )
    lines.append("")
    ...
    lines.append("-" * 100)
    lines.append(
        f"  {'CAMPAIGN':<38s} {'CPC':>6s} {'CTR':>6s} {'SPEND':>10s} {'ORDER':>6s} {'KOMISI':>10s} {'ROI':>6s} {'LABEL':>12s}"
    )
    lines.append("-" * 100)
    ...
    for cid, c in sorted_camps:
        ...
        comm_s = (
            f"Rp {c['shopee_commission']:,.0f}"
            if c["shopee_commission"] > 0
            else "Rp 0"
        )
        ...
        comm_str = (
            f"Rp {c['shopee_commission']:,.0f}" if c["shopee_commission"] > 0 else "-"
        )
        roi_str = f"{c['roi']:.1f}x" if c["roi"] > 0 else "-"

        lines.append(
            f"  {name:<38s} {cpc_str:>6s} {ctr_str:>6s} {spend_str:>10s} {order_str:>6s} {comm_str:>10s} {roi_str:>6s} {c['label']:>12s}"
        )

    lines.append("-" * 100)
    lines.append(
        f"  TOTAL: {len(sorted_camps)} campaigns | Spend: Rp {total_spend:,.0f} | Komisi: Rp {total_comm:,.0f} | ROI: {total_roi:.2f}x"
    )
    lines.append("=" * 100)
    ...
    if busuk_camps:
        ...
        for cid, c in busuk_camps[:10]:
            ...
            lines.append(
                f"     ❌ {c['name'][:55]} | CPC: Rp{c['cpc']:.0f} | CTR: {c['ctr']:.1f}% | Spend: Rp {c['spend']:,.0f}"
            )
...
def main():
    ...
    filename = os.path.join(
        REPORT_DIR, f"campaign_roi_report_{datetime.now().strftime('%Y%m%d_%H%M')}.txt"
    )
