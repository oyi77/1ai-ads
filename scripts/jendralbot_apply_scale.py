...
if not POSTBRIDGE_API_KEY:
    ...
    # Ask user for input
    user_key = input(
        "\nOr paste API key here (or 'skip' to continue without API): "
    ).strip()
...
instagram_orders = next(
    (p for p in scale_plan["platform_budget"] if p == "Instagram"), None
)
...
for product in monitor_products[:3]:
    show_action(
        "MONITOR", product.get("product", "Unknown"), "Keep active, monitor performance"
    )
...
for product in pause_products[:5]:
    show_action(
        "PAUSE", product.get("product", "Unknown"), "Stop campaigns, review product"
    )
