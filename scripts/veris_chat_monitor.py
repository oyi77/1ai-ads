...
STATE_FILE = os.path.expanduser(
    "~/.openclaw/workspace/.vilona/veris_monitor_state.json"
)
...
async def send_report(
    client, summaries, total_unread, total_mentions, new_activity, check_num
):
...
async def main_loop():
    ...
    while True:
        try:
            ...
            # Report
            log(
                f"📊 Check #{len(state['total_unread_history'])} | Total unread: {total_unread} | Mentions: {total_mentions} | Chats w/ unread: {len(summaries)}"
            )
            ...
            if new_activity:
                ...
                for a in new_activity[:15]:  # Max 15 shown
                    log(
                        f"   📨 {a['name']}: +{a['new_since_last']} messages (total unread: {a['unread']}, mentions: {a['mentions']})"
                    )
            ...
            if summaries:
                ...
                for s in summaries[:10]:
                    ...
                    type_flag = (
                        "👤" if s["is_user"] else ("📢" if s["is_channel"] else "👥")
                    )
                    log(
                        f"   {type_flag} {s['name']}: {s['unread']} unread{mention_flag}"
                    )
