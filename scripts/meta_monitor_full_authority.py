...
def scale_horizontal(adset_id, adset_name):
    ...
    try:
        ...
        if new_id:
            # Rename for tracking
            requests.post(f'https://graph.facebook.com/v19.0/{new_id}', 
                          params={'name': f'SCALED_DUP_{adset_name}_VILONA', 'status': 'ACTIVE', 'access_token': ACCESS_TOKEN})
            log_action(f'🚀 HORIZONTAL SCALE: Duplicated Adset {adset_name} -> {new_id} (VILONA MODE)')
    ...
    for c in camps:
        ...
        if ins['ctr'] > 7.0 and ins['cpr'] < 120 and budget < 2000000:
            ...
            requests.post(f'https://graph.facebook.com/v19.0/{c["id"]}', 
                          params={'daily_budget': new_budget, 'name': new_name, 'access_token': ACCESS_TOKEN})
            log_action(f'💰 VERTICAL SCALE: Campaign {new_name} ({budget} -> {new_budget}) CTR:{ins["ctr"]:.2f}% CPR:{ins["cpr"]:.0f}')

    # 2. LEVEL ADSET (Horizontal Scaling & Killing)
    url_as = f'https://graph.facebook.com/v19.0/{AD_ACCOUNT_ID}/adsets'
    adsets = requests.get(url_as, params={'access_token': ACCESS_TOKEN, 'fields': 'id,name,status,campaign{name}'}).json().get('data', [])
    
    for as_ in adsets:
        if as_['status'] != 'ACTIVE': continue
        ins = get_insights(as_['id'], 'adset')
        if not ins or ins['spend'] < 10000: continue
        
        # KILL LOST ADSETS
        if ins['cpr'] > 160:
            requests.post(f'https://graph.facebook.com/v19.0/{as_["id"]}', params={'status': 'PAUSED', 'access_token': ACCESS_TOKEN})
            log_action(f'💀 KILLED ADSET: {as_["name"]} in {as_["campaign"]["name"]} (CPR: {ins["cpr"]:.0f})')
        
        # H-SCALE (Super Winning Adset)
        if ins['ctr'] > 10.0 and ins['cpr'] < 90 and ins['spend'] > 50000:
            log_action(f'💎 SUPER WINNER DETECTED: {as_["name"]} - Triggering Horizontal Scale...')
            scale_horizontal(as_['id'], as_['name'])

    # 3. LEVEL ADS (Micro-Optimization)
    url_ads = f'https://graph.facebook.com/v19.0/{AD_ACCOUNT_ID}/ads'
    ads = requests.get(url_ads, params={'access_token': ACCESS_TOKEN, 'fields': 'id,name,status'}).json().get('data', [])
    for ad in ads:
        ...
        # KILL POOR CREATIVES
        if ins['ctr'] < 4.0:
            requests.post(f'https://graph.facebook.com/v19.0/{ad["id"]}', params={'status': 'PAUSED', 'access_token': ACCESS_TOKEN})
            log_action(f'✂️ KILLED CREATIVE: {ad["name"]} (CTR Lazy: {ins["ctr"]:.2f}%)')
